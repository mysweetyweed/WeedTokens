...
const string strMessageMagic = "WeedTokens Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, const int nHeight)
{
    ...
    if (nHeight == 1) //10% premine
    {
      nSubsidy = 1000000 * COIN;
    } 
    else if (nHeight > 2 && nHeight <= 200) //pervent instamine
    {
        nSubsidy = 0 * COIN;
    }
    else if (nHeight > 200 && nHeight <= 1540) //268000 coins
    {
        nSubsidy = 200 * COIN;
    }
    else if (nHeight > 1540 && nHeight <= 2980) //576000 coins
    {
        nSubsidy = 400 * COIN;
    }
    else if (nHeight > 2980 && nHeight <= 10080) //~8156000 for fair distribution
    {
        nSubsidy = 1150 * COIN;
    }
    else if (nHeight > 10080 && nHeight <= 10280) //if blockchain get stuck at switching to pos this will help
    {
        nSubsidy = 0 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WeedTokens", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 20 * 60; // test net min age is 20 minutes
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "First Social Network for Cannabis followers, share your experience and have fun.";
        CTransaction txNew;
        txNew.nTime = 1421225489;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1421225489;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 449206 : 449206;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("4d553e4ff66d871e0f16ca3081e7a11d022d25b34cc9515f2aab8eb293c280a5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe8, 0x35, 0xe5, 0x05 };
